...
async function hasWebAuthn() {
  ...
  return !!((windowsVersion < 0 || windowsVersion >= 11) && (safariVersion < 0 || safariVersion >= 14) && !isMobileFirefox() && await getWebAuthnSupport());
}
...
function FfiHasWebAuthn() {
  return new Promise(async (resolve) => {
    try {
      let webauthn2 = await hasWebAuthn();
      resolve(webauthn2);
    } catch (err) {
      resolve(false);
    }
